...
#Dimension Creation
Creates dimdep dimanx scaredytot dimafflaby dimafflab dimirrit dimpsych dimbs dimsleep 
### Dimension Anxiety
```{r}
...
FOR <- FOR %>%
  mutate(
    polye = rowMeans(
      cbind(malts, maltp, malte, maltn, maltv, bullying,
            rsesmedul, rsesfedul, rsesown, rsesincome),
      na.rm = TRUE
    )
  )
```
...
for (dx in KSADS) {
  ...
  FOR <- FOR %>%
    group_by(subject_id) %>%
    arrange(assessment_date, .by_group = TRUE) %>%
    mutate(!!o_var := ifelse(row_number() == which(.data[[cc_var]] == 1)[1] & time_point >= 2 & .data[[cc_var]] == 1, 1, 0)) %>%
    ungroup()
}
```
